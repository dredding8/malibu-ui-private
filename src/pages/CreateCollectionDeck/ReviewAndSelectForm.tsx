...
interface ReviewAndSelectFormProps {
  data: any;
...
// Sample data with duration and site fields for summary
const sampleMatches: Match[] = [
  {
    ...
    notes: '',
    selected: true, // Pre-selected (Optimal)
    needsReview: false,
    unmatched: false,
    duration: 45,
    site: 'THU'
  ...
  },
  {
    ...
    notes: '',
    selected: false, // Not pre-selected (Baseline)
    needsReview: true,
    unmatched: false,
    duration: 22,
    site: 'CLR'
...
const ReviewAndSelectForm: React.FC<ReviewAndSelectFormProps> = ({
  data,
  onUpdate,
  onBack,
  onCancel
  ...
  // Simulate loading with smart defaults
  const simulateLoading = useCallback(async () => {
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      ...
      // Smart default: Pre-select all "Optimal" matches
      const optimalMatches = sampleMatches.filter(m => m.match === 'Optimal');
  ...
  const handleCreateCollection = async () => {
    ...
    try {
      ...
      if (response && response.id) {
        // Navigate to Success Screen with collection ID
  ...
  return (
    <div>
      <h3 id="step-heading">Step 2: Review & Select Opportunities</h3>
      ...
      {isGenerating ? (
        <Card className="bp6-margin-top">
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <h4 className="bp6-margin-top">Finding Collection Opportunities</h4>
            <Text className="bp6-margin-top" style={{ color: '#666', marginBottom: '20px' }}>
              {loadingStep}
            </Text>
            <ProgressBar
              ...
              value={progress / 100}
            />
            <Text className="bp6-margin-top" style={{ color: '#666' }}>
...
export default ReviewAndSelectForm;
